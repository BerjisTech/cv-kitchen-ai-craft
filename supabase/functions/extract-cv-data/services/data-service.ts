/**
 * Checks if there is existing extracted data for a document and deletes it to force re-extraction
 */
  ...
  try {
    ...
    if (existingDataResponse.ok) {
      ...
      if (existingData && existingData.length > 0) {
        console.log(`Found existing extracted data for document ${documentId} - deleting it to force re-extraction`);
        
        // Delete the existing entry to force re-extraction
        await fetch(
        ...
        console.log(`Successfully deleted existing extracted data for document ${documentId}`);
      }
    } else {
      console.error("Error checking for existing data:", await existingDataResponse.text());
    }
    return null; // Always return null to force re-extraction
  } catch (err) {
